...
docker run --rm --net="${DOCKER_NETWORK}" \
  ...
  -v "${USER_TOKEN_PATH}:/etc/token/ntoken" \
  -v `pwd`/zms/var/certs/zms_cert.pem:/etc/certs/zms_cert.pem \
  -v `pwd`/acceptance-test/public.pem:/etc/certs/public.pem \
  --name athenz-zms-cli athenz-zms-cli \
  -f /etc/token/ntoken \
  -z "https://${ZMS_HOST}:${ZMS_PORT}/zms/v1" -c /etc/certs/zms_cert.pem \
...
docker run --rm --net="${DOCKER_NETWORK}" \
  ...
  -v "${USER_TOKEN_PATH}:/etc/token/ntoken" \
  -v `pwd`/zms/var/certs/zms_cert.pem:/etc/certs/zms_cert.pem \
  -v `pwd`/ui/keys/athenz.ui-server_pub.pem:/etc/certs/athenz.ui-server_pub.pem \
  --name athenz-zms-cli athenz-zms-cli \
  -f /etc/token/ntoken \
  -z "https://${ZMS_HOST}:${ZMS_PORT}/zms/v1" -c /etc/certs/zms_cert.pem \
...
docker run --rm -v `pwd`/acceptance-test:/etc/acceptance-test \
  --name athenz-cli-util athenz-cli-util \
  ./utils/zms-svctoken/target/linux/zms-svctoken \
  -domain garm -service tester \
  -key-version tmp \
  -private-key /etc/acceptance-test/private.pem > `pwd`/acceptance-test/n-token

### checking:
less ./acceptance-test/n-token
```
...
docker run --rm --net="${DOCKER_NETWORK}" \
  -v `pwd`/zts/var/certs/zts_cert.pem:/etc/ssl/certs/ca-certificates.crt \
  -v `pwd`/acceptance-test:/etc/acceptance-test \
  --name athenz-cli-util athenz-cli-util \
...
docker run --rm --net="${DOCKER_NETWORK}" \
  -v `pwd`/zts/var/certs/zts_cert.pem:/etc/certs/zts_cert.pem \
  -v `pwd`/acceptance-test:/etc/acceptance-test \
  --name athenz-cli-util athenz-cli-util \
...
### checking:
openssl x509 -text -noout -in ./acceptance-test/service.crt | less
openssl x509 -text -noout -in ./acceptance-test/intermediate.crt | less
openssl verify -CAfile ./zts/var/keys/zts_cert_signer_cert.pem ./acceptance-test/service.crt
### expect:
### ./acceptance-test/service.crt: OK

### checking:
docker run --rm --entrypoint curl \
  --ip "${CLI_STATIC_IP}" \
  --network="${DOCKER_NETWORK}" \
  -v `pwd`/acceptance-test/service.crt:/etc/certs/service.crt \
  -v `pwd`/acceptance-test/private.pem:/etc/certs/private.pem \
  --name athenz-curl appropriate/curl \
...
ZTS_CERT_SIGNER_CA_PATH=${ZTS_CERT_SIGNER_CA_PATH:-./acceptance-test/intermediate.crt}
ZTS_SSL_TRUSTSTORE_PATH=${ZTS_SSL_TRUSTSTORE_PATH:-./zts/var/certs/zts_truststore.jks}
...
### checking:
openssl x509 -text -noout -in ./acceptance-test/role.crt | less
openssl verify -CAfile ./zts/var/keys/zts_cert_signer_cert.pem ./acceptance-test/role.crt
### expect:
### ./acceptance-test/role.crt: OK

### checking:
docker run --rm --entrypoint curl \
  --ip "${CLI_STATIC_IP}" \
  --network="${DOCKER_NETWORK}" \
  -v `pwd`/acceptance-test/role.crt:/etc/certs/role.crt \
  -v `pwd`/acceptance-test/private.pem:/etc/certs/private.pem \
  --name athenz-curl appropriate/curl \
...
```bash
# get policy - ZPU
rm -f ./acceptance-test/zpu.log
mkdir -p ./acceptance-test/zpu/metrics
docker run --rm --net="${DOCKER_NETWORK}" \
  -v `pwd`/zts/var/certs/zts_cert.pem:/etc/certs/zts_cert.pem \
  -v `pwd`/acceptance-test:/etc/acceptance-test \
  -v `pwd`/acceptance-test/zpu:/home/athenz/tmp/zpe \
  -v `pwd`/zts/conf/athenz.conf:/tmp/athenz.conf \
  --name athenz-cli-util athenz-cli-util \
...
### checking:
less ./acceptance-test/zpu.log
# cat ./acceptance-test/zpu/garm.pol
```
